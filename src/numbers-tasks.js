...
function getAngleBetweenVectors(x1, y1, x2, y2) {
  // throw new Error('Not implemented');
  const string1 = x1 * x2 + y1 * y2;
  const string2 = Math.sqrt(x1 ** 2 + y1 ** 2) * Math.sqrt(x2 ** 2 + y2 ** 2);
  return Math.acos(string1 / string2);
}
...
function toNumber(value, def) {
  ...
  if (typeof +value === 'number' && !value === false && !isNaN(value)) {
    return value;
  }
  return def;
}
